...
module.exports = {
  ...
  async doTask(task) {
    try {
      ...
      let res = await rp.get({
        uri: `http://yxpjwnet.com/page/${task.params.page}.html`,
        headers,
        encoding: null,
        gzip: true,
      ...
      $('article').each((i, ele) => {
        task.result.push({
          type: $('h2>a', ele).prop('href').split('/')[1],
  ...
  async success(task, tm) {
    ...
    await mongo.persist(async (client) => {
      for (let i in task.result) {
        ...
        }, task.result[i], {upsert: true});
        if (result.upsertedCount == 0) {
          needNextPage = false;
          logger.error(`${task.result[i].title} ${task.result[i].time} already added`);
        } else {
          // add detail task
      ...
      if (needNextPage && task.result.length > 0 && task.params.page < 2) {
        await tm.addTask({
          type: task.type,
          groups: task.groups,
          params: {
  ...
  group: {
    async done(group, tm) {
      await mongo.persist(async (client) => {
        await client.collection('fuli_group').insertOne(group);
      })
    }
  }
};
