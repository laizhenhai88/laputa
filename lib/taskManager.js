...
class TaskManager extends EventEmitter {
    ...
    async addTask(task) {
        ...
        await dbPool.persist(async(client)=> {
    ...
    async _taskSuccess(worker, task) {
        ...
        dbPool.persist(async(client) => {
            // 执行任务模块回调,注意此处应为无事务性,所以允许任务重复,但不允许任务掉链子
            await require('./worker/' + task.type).success(task, this);
            // 持久化任务状态
            await client.collection('task').updateOne({_id: dbPool.ObjectID(task._id)}, {
                $set: {
                    status: task.status,
                    result: task.result,
                    finishTime: new Date()
    ...
    async _taskFailed(worker, task) {
        ...
        dbPool.persist(async(client) => {
            // 更新db中任务状态,记录log,预警
            task._id = dbPool.ObjectID(task._id);
