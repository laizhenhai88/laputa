...
class TaskManager extends EventEmitter {
  ...
  async _load() {
    await mongo.persist(async (client) => {
      let tasks = await client.collection('task').find({
        status: 'init'
      }).toArray();
      for (let i in tasks) {
  ...
  _addTask(task, prepend) {
    ...
    delay = offsetDelay < 0 ? 0 : offsetDelay;
  ...
  async _taskGroupIncrease(task) {
    // 如果有任务组，则进行记录
    if (task.group) {
      let group = this.groups.get(task.group.id);
      if (group) {
        group.count++;
      } else {
        this.groups.set(task.group.id, {count: 1, done: task.group.done, createTime: new Date()});
      }
    }
  }
  ...
  async _taskGroupDecrease(task) {
    ...
    if (task.group) {
      ...
      if (group.count == 0) {
        // 任务组完成
        try {
          this.groups.delete(task.group.id);
          await require('./worker/' + task.group.done).group.done(group, this);
        } catch (e) {
