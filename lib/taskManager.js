...
class TaskManager extends EventEmitter {
  ...
  delayTaskAnalyze() {
    let types = [];
    this.delayTasks.forEach((delay, task) => {
  ...
  runDelayTaskNow(taskid) {
    let task = undefined
    this.delayTasks.forEach((k, v) => {
      if (v._id.toString() == taskid) {
        task = v
      }
    })
    if (task) {
      clearTimeout(task[taskTimeoutId])
  ...
  async addTask(task, prepend) {
    ...
    try {
      ...
      task.result = 'beforeAddTask error' + e.message
    }

    if (!shouldAddTask) {
      logger.error(`should not add task ${JSON.stringify(task)}`);
      if (!task.result) {
        task.result = 'should not add task'
      }
      task.status = 'failed'
  ...
  _addTask(task, prepend) {
    ...
    task[taskTimeoutId] = setTimeout(async () => {
      this.delayTasks.delete(task);
      if (prepend) {
        this.tasks.push(task);
      } else {
  ...
  async _taskSuccess(worker, task) {
    ...
    await mongo.persist(async (client) => {
      ...
      await client.collection('task').deleteOne({
        _id: mongo.ObjectID(task._id)
      });
      ...
      for (let i in task.groups) {
        ...
        if (group.count == 0) {
          ...
          try {
            ...
            await require('./worker/' + task.groups[i].done).done(group, this);
            logger.info(`group ${JSON.stringify(group)} done`);
          } catch (e) {
